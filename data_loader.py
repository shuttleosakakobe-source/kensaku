...
def maintenance_admin_screen():
    ...
    with tab1:
        ...
        # 状態保持用 Session State の初期化
        if "maint_cust_code" not in st.session_state:
            st.session_state.maint_cust_code = ""
        if "maint_cust_name" not in st.session_state:
            st.session_state.maint_cust_name = ""
        if "maint_store_code" not in st.session_state:
            st.session_state.maint_store_code = ""
        if "maint_store_name" not in st.session_state:
            st.session_state.maint_store_name = ""
