...
def render_route_change_tabs():
    ...
    def _tab6_body():
        ...
        f_cust_code = col_f1.text_input("顧客コード", key="r_tab_search_cust_code")
        f_applicant = col_f2.text_input("担当者名", key="r_tab_search_applicant")
        f_date_type = col_f3.selectbox("期間の基準日", ["申請日", "処理日"], key="r_tab_search_date_type")

        col_d1, col_d2 = st.columns(2)
        f_date_from = col_d1.date_input("開始日", value=None, key="r_tab_search_date_from")
        f_date_to = col_d2.date_input("終了日", value=None, key="r_tab_search_date_to")
    ...
    _r_tab_visible_nums = [_n for _n in range(1, 7) if tab_visible(_n)]
    if not _r_tab_visible_nums:
                    ...
                    for idx, row in approved_df.iloc[::-1].iterrows():
                        ...
                        with st.expander(f"🟢【{_v('cust_name')}（{_v('cust_code')}）】 承認者: {mgr_name}"):
                            ...
                            staff_comment_val = st.text_area(
                                "コメント", key=f"rt_staff_comment_{row_id}",
                                placeholder="差戻しにはせず、申請者に伝えたい連絡事項があれば入力してください",
                            )
                            if st.button("💬 コメントを送信", key=f"rt_staff_comment_btn_{row_id}"):
