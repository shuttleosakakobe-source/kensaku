...
# --- 設定値 ---
GAS_URL = "YOUR_GAS_WEB_APP_URL"  # ご自身のGASウェブアプリURLに置き換えてください
TARGET_SHEET_URL = "https://docs.google.com/spreadsheets/d/YOUR_SOURCE_SHEET_ID/edit" # 元スプレッドシートの編集URL
TARGET_SHEET_CSV = "https://docs.google.com/spreadsheets/d/YOUR_SOURCE_SHEET_ID/gviz/tq?tqx=out:csv" # 元スプレッドシートのCSV出力URL
DEST_SHEET_URL = "https://docs.google.com/spreadsheets/d/1iiiCnlP0_wLgIJ092qiorb-Dj4O1GwNt_J9z92VXQNI/edit?gid=0#gid=0" # 業務担当用転記先URL

st.set_page_config(page_title="メンテナンス申請・処理システム", layout="wide")
                        ...
                        with st.form(key=f"mgr_form_{row_id}"):
                            ...
                            if btn_approve or btn_reject or btn_delete:
                                ...
                                if btn_approve:
                                    ...
                                    updated_row.extend([mgr_name, now_str, comment]) # AC: 管理職名, AD: 時刻, AE: コメント
                                elif btn_reject:
...
with tab3:
    ...
    try:
        ...
        if df.empty or len(df.columns) < 29:
            ...
            # AC列（インデックス28）が「申請中/差戻し/削除/nan/業務転記済」以外の承認済みデータを取り出し
                    ...
                    with st.expander(expander_label):
                        ...
                        with st.form(key=f"transfer_form_{row_id}"):
                            ...
                            if btn_transfer:
                                ...
                                # 元データ1行分を空欄補正して取得し、末尾に「業務転記日時」「業務担当者名」「業務メモ」を追加
