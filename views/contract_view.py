...
def render_contract_change_tabs():
    ...
    def _tab6_body():
        ...
        f_cust_code = col_f1.text_input("顧客コード", key="c_tab_search_cust_code")
        f_applicant = col_f2.text_input("担当者名", key="c_tab_search_applicant")
        f_date_type = col_f3.selectbox("期間の基準日", ["申請日", "処理日"], key="c_tab_search_date_type")

        col_d1, col_d2 = st.columns(2)
        f_date_from = col_d1.date_input("開始日", value=None, key="c_tab_search_date_from")
        f_date_to = col_d2.date_input("終了日", value=None, key="c_tab_search_date_to")

        if st.button("🔍 検索する", key="c_tab_search_btn"):
            try:
                read_csv_cached.clear()
                df_search = read_csv_cached(CC_DEST_SHEET_CSV)
    ...
    _c_tab_visible_nums = [_n for _n in range(1, 7) if tab_visible(_n)]
                        ...
                        with st.expander(f"🟢【{_v('cust_name')}（{_v('cust_code')}）】 承認者: {mgr_name}"):
                            ...
                            with st.form(key=f"cc_transfer_form_{row_id}"):
                                ...
                                staff_comment_val = st.text_area(
                                    "💬 申請者への連絡コメント（任意・差戻しにはなりません）", key=f"cc_staff_comment_{row_id}",
