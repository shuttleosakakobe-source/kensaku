...
def render_temp_delivery_form():
    ...
    with col_code_lbl:
        st.markdown("**顧客コード**")
    with col_code_val:
        code_options = [""] + list(customers.keys())
        selected_code = st.selectbox("顧客コード選択", options=code_options, label_visibility="collapsed")

    selected_info = customers.get(selected_code, {"store_name": "", "store_code": "", "cust_name": ""})

    col_sname_lbl, col_sname_val, col_scode_lbl, col_scode_val = st.columns([1.5, 3, 1.5, 2])
    with col_sname_lbl:
        st.markdown("**加盟店名**")
    with col_sname_val:
        st.info(selected_info["store_name"] if selected_info["store_name"] else " ")
        
    with col_scode_lbl:
        st.markdown("**加盟店コード**")
    with col_scode_val:
        st.info(selected_info["store_code"] if selected_info["store_code"] else " ")

    col_cname_lbl, col_cname_val = st.columns([1.5, 6.5])
    with col_cname_lbl:
        st.markdown("**顧客名**")
    with col_cname_val:
        st.info(selected_info["cust_name"] if selected_info["cust_name"] else " ")
    ...
    edited_df = st.data_editor(
        default_df,
        column_config={
            "品記号": st.column_config.TextColumn("品記号", width="medium"),
            "数量": st.column_config.NumberColumn("数量", min_value=0, step=1, default=0),
            "単価": st.column_config.NumberColumn("単価", min_value=0, step=1, default=0),
            "伝票出力": st.column_config.SelectboxColumn("伝票出力", options=["有", "無"], required=True)
        },
        num_rows="fixed",
    ...
    with col_date:
        delivery_date = st.date_input("📅 納品日", value=get_jst_today() + timedelta(days=1))
    with col_route:
        route_code = st.text_input("🚚 納品ルートコード", placeholder="例: R-01")

    if st.button("🚀 管理職チェックへ申請する", type="primary", use_container_width=True):
        ...
        with st.spinner("スプレッドシートへ送信中..."):
            ...
            if res.get("status") == "success":
                st.success("✅ 臨時納品の申請を送信しました！")
                st.rerun()
            else:
                st.error(f"❌ 送信エラー: {res.get('message', '通信失敗')}")
...
def maintenance_admin_screen():
    ...
    if st.session_state.get("user_role") not in ["0", "3"]:
        st.error("⚠️ この画面へのアクセス権限がありません。")
    ...
    st.write("### 🛠️ メンテナンス管理システム")
    
    tab1, tab2, tab3, tab4 = st.tabs(["📝 申請入力", "🔍 管理職チェック", "⚙️ 処理一覧", "🖨️ 印刷用"])
    
    with tab1:
        maint_type = st.selectbox(
            "⚙️ メンテナンスの種類を選択",
            ["臨時納品", "単発ルート変更", "納品数量", "変更ルート・担当者変更", "契約内容変更", "期間ストップ", "客中残訂正", "その他"]
        )
        st.write("---")
        
        if maint_type == "臨時納品":
            render_temp_delivery_form()
        else:
            st.info(f"「{maint_type}」の入力フォームは順次実装予定です。")

    with tab2:
        st.write("#### 🔍 未チェック一覧（管理職専用）")
        st.caption("申請されたデータをここで確認・チェックします。")
        
    with tab3:
        st.write("#### ⚙️ 処理一覧")
        st.caption("チェック済みのデータ処理を行います。")
        
    with tab4:
        st.write("#### 🖨️ 印刷用データ")
        st.caption("帳票用の印刷データを確認します。")
...
def main_screen():
    ...
    if st.session_state.user_role in ["0", "3"]:
        ...
        st.caption("※ 従来のスプレッドシート版は下記よりアクセス可能です。")
